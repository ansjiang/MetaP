...
class Graph:
	...
	def build_graph(self, mapfile='./tol_species.txt'):
		self.node_count = 0
		my_map = []
		my_list=[]
		layer = -1
	...
	def get_map(self, permute=-1):
		...
		map_height = self.layers
		if(map_width % 3 != 0) :
			map_width = map_width + 3 - map_width % 3
	...
	def get_dense_map(self, permute=-1):
		map_width = self.width
		map_height = self.layers
		if(map_width % 3 != 0) :
			map_width = map_width + 3 - map_width % 3
		...
		for i in range(0, self.layers):
			...
			for n in current:
				m[i][j] = n.get_abundance()
				if(len(n.get_children()) == 0) :
					leavesStartLayer = min(leavesStartLayer , i) 
					
				temp = np.append(temp, n.get_children())
				j = j+1
			current = temp
		
		return m[leavesStartLayer - 1: , :]
